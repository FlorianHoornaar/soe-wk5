...
let addFirstPerson = () => {
    if (firstPerson === "aaa") {
        ...
        people = [...people, firstPerson.trim()].sort();
...
let addPerson = () => {
    if (newPerson.trim().length > 0) {
        if (people.includes(newPerson.trim()) === false) {
            people = [...people, newPerson.trim()].sort();
        }
        newPerson = ""
    }
}
...
let addAchievement = () => {
    if (newAchievement.trim().length > 0) {
        achievements = [...achievements, newAchievement.trim()].sort();
        newAchievement = ""
    }
}
...
<Form>
    <FormGroup>
        <Label for="Name"><h1>Hi {firstPerson}! Who else is on your team?</h1></Label>
        <Input
